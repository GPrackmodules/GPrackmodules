...
float ChorusDelayLine::Read(int nChannel, float fDelayS)
{
	...
	for (int i =0; i < N_TAPS; i++)
		fReturn += (*(pDelayLine++)) * (*(pIR++));	// dot product = FIR convolution (if FIR is in reverse order)
	return fReturn;
}
...
{
	//
	// Step 1: make a huge lowpass with nFilterTaps * nSubSample taps at (relative)fCutoffFrequency / nSubSample
	//
	...
	//
	// Step 2: Make nSubSample impulse responses, starting at all offset from nSubSample -1 to 0,
	// to produce the IRs for delays between 0 and 1 sample
	//
	...
	for (size_t nSub = 0; nSub < N_SUBSAMPLE; nSub++)
	{
		...
		{
			// invert the order of the FIR response, so we can do the discreet convolution with dot product
